...
| [0007](adr/0007-maximum-file-size.md) | A regular file is at most 2^20 chunks long; `vfs.FS.MaxFileSize` declares the limit, FSINFO advertises it, and a size or write past it is refused before anything is touched | none — it constrains `internal/blobfs` and `internal/nfs`, and adds `MaxFileSize` to the `vfs.FS` contract, which the backend replacing `blobfs` (section 14) implements too |
